...
class RewardModelWorker(Worker):
    ...
    def __init__(self, config):
        ...
        if self.config.reward_manager == 'instruction_rb':
        ...
        with torch.no_grad(), torch.autocast(device_type='cuda', dtype=torch.bfloat16):
            ...
            if self.use_remove_padding:
                ...
                output = self.reward_module(input_ids=input_ids,
                                            ...
                                            use_cache=False)
                if self.instruction:
                    rm_score = output  # (batch_size, seq_len, 1)
                else:
    ...
    def _switch_chat_template(self, data: DataProto):
        ...
        for i in range(data.batch.batch_size[0]):
            # extract raw prompt
            if self.instruction:
                chat= [data.non_tensor_batch['raw_prompt_with_chat_template'][i]]
            elif isinstance(data.non_tensor_batch['raw_prompt'][i], list):
    ...
    def compute_rm_score(self, data: DataProto):
        ...
        if self._do_switch_chat_template or self.instruction:
